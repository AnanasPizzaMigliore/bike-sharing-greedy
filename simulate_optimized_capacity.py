...
from optimize_capacity import optimize_capacity
import pdb
...
total_trip = np.zeros((7))
total_rebalance = np.zeros((7))
...
#h = 0 #starting hour
#start_date = np.array([h, weekday, 0, month, year]).astype(int)
#print(f'the start date: {start_date}')
#state, _, survival = optimal_states(p_th,state,capacities,departures,arrivals,Tr,Tp,start_date,survival_mat,False)
...
for day in range(days):
    for hour in range(24):
        ...
        optimal_state, survival, max_survival = optimal_states(p_th,state,capacities,departures,arrivals,Tr,
                                                               Tp,date,survival_mat,day==days)
        ...
        bikes_numbers = state - optimal_state
        state, trucks, distance, count, trip_count, rebalance_count = optimize_capacity(state, survival, max_survival, optimal_state, beta, gamma, capacity)
        #print(f'trip count {trip_count}')
        station_count = station_count + count
        #print(distance)
        total_trip[weekday] = total_trip[weekday] + trip_count
        total_distance[weekday] = total_distance[weekday] + distance
        total_rebalance[weekday] = total_rebalance[weekday] + rebalance_count
...
print(f'year: {year}, month: {month}')
    ...
    #np.save(strcat('results_', num2str(year), '_', num2str(month), '_unopt.mat'));
np.savez('Results/results_' + str(year) + '_' + str(month) + '_'  + str(beta) + '_' + str(gamma) + '_' +str(capacity), 
         total_full = total_full, total_empty = total_empty, failures = failures, sent_trucks = sent_trucks, 
         total_distance = total_distance, station_count = station_count, total_trip = total_trip, total_rebalance = total_rebalance)
