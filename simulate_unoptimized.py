...
h = np.floor(np.mod(second[trip],sec_per_day) /3600) #starting hour
...
for day in range(days):
    for hour in range(24):
        ...
        trucks = 0
            ...
            #[state, trucks, distance] = optimize(state, survival, max_survival, optimal_state, beta, gamma);
        #total_distance = total_distance + sum(distance)
        ...
        while ((trip < len(second) and second[trip] < time_offset) or ( len(traveling) != 0 and traveling[0][0] < time_offset)):
            ...
            if ( len(traveling) != 0 and ( trip >= len(second) or traveling[0][0] < second[trip])):
                ...
                if(state[station] == 0):
                    ...
                    #print(f'arrival time {arrival_time}, arrival station {arrival_station} 
                    #value {arrival_station} length {len(arrival_station)}\n')
...
print(f'year: {year}, month: {month}')
    ...
    #np.save(strcat('results_', num2str(year), '_', num2str(month), '_unopt.mat'));
np.savez('Results/results_' + str(year) + '_' + str(month) + '_unopt', total_full = total_full, total_empty = total_empty, 
         failures = failures,sent_trucks = sent_trucks, total_distance = total_distance)
